...
**Current Milestone:** Two parallel tracks, per CTO directive (2026-07-23). **Track A** continues ACA-MVP-001 (`docs/11_mvp/ACA-MVP-001.md`) — the first experiments in this program to use a real Transformer rather than an MLP. Benchmark A (memory/ME-03), complete including its stretch ablation (Section 8): EXP-018 found competence-gated memory (validated at toy scale by EXP-001) gives **zero measurable protection** against catastrophic forgetting under staged, non-rehearsed continual training — memory coverage of the oldest facts was exactly 0.0 at evaluation time, every seed. EXP-010 then tested the most direct candidate fix (a one-time consolidation-replay burst) — **also failed** (0.160 ± 0.028 vs. 0.158 ± 0.027 baseline). Benchmark B (compose/RC-01), complete (Section 9): tested on the real, published SCAN `addprim_jump` split (grammar verified against all 22,376 real examples first) — a 326-parameter structure-matched model scored **100.000% ± 0.000%** exact-match, against **0.71% ± 0.39%** for a 681,481-parameter generic Transformer (closely matching Lake & Baroni's own published ~1% baseline) — a **~141x margin**, decisively clearing the pre-registered 2x threshold. Benchmark C (composability, Section 10), complete: one joint model combining both pathways, disjoint parameters, one optimizer — **no measurable interference** (recall 0.152 ± 0.025 vs. 0.158 ± 0.027 isolated; compose 1.0000 ± 0.0000, exact match to isolated), extending EXP-004's toy-scale composability finding to real scale. **ACA-MVP-001's originally-scoped Benchmark A/B/C order is now complete** — three independently-earned outcomes (memory negative, compose decisively positive, composability confirmed), none averaged into the others. Full mechanisms: `docs/06_experiments/Completed.md` (EXP-018, EXP-010, EXP-020, EXP-021). **Track B** produced CTX-001 (`docs/12_cognition/CTX-001.md`): investigates whether EXP-018/010, conversation-context representation, and "known unknown" tracking share a root cause. Rejects the strong version of that idea (relabeling "information" as "cognitive state" doesn't fix EXP-018's failure) but finds a narrower, defensible version — the shared root is retention/allocation policy under shared, capacity-constrained storage. Its tentative fourth-substrate proposal ("S_working") was then directly tested per CTO directive: **EXP-019 falsified it** — a reserved capacity partition within the existing S_episodic mechanism, written unconditionally, performs identically (1.000 ± 0.000 recall) to full structural separation, while naive schema-tagging collapses (0.200 ± 0.100, zero coverage) via a newly-identified **write-starvation** mechanism, distinct from EXP-018's eviction-based one. No new substrate was added to ACA v1.0. `docs/13_state_model/SOS-001.md` now catalogs every internal state object's owner, lifecycle, and boundaries in one place — the standing reference for IVS-001's still-unexecuted Stage 3/4 integration.
