...
**Current Milestone:** Two parallel tracks, per CTO directive (2026-07-23). **Track A** continues ACA-MVP-001 (`docs/11_mvp/ACA-MVP-001.md`): Benchmark A, complete including its stretch ablation (Section 8) — the first experiments in this program to use a real Transformer rather than an MLP. EXP-018 tested whether competence-gated memory (ME-03, validated at toy scale by EXP-001) protects old facts from catastrophic forgetting under staged, non-rehearsed continual training: **the pre-registered success criterion (≥2x baseline retention) failed cleanly** — memory coverage of the oldest facts was exactly 0.0 at evaluation time, in every seed, for both naive and competence-gated policies. EXP-010 then tested the most direct candidate fix (a one-time consolidation-replay burst at each stage boundary) — **also failed** (0.160 ± 0.028 vs. 0.158 ± 0.027 baseline, no improvement). Both mechanisms fully identified, not just observed: `docs/06_experiments/Completed.md`. Benchmark B (SCAN, real published compositional-generalization split) is in progress. **Track B** produced CTX-001 (`docs/12_cognition/CTX-001.md`): investigates whether EXP-018/010, conversation-context representation, and "known unknown" tracking share a root cause. Rejects the strong version of that idea (relabeling "information" as "cognitive state" doesn't fix EXP-018's failure) but finds a narrower, defensible version — conversation/reasoning context is the one candidate that doesn't cleanly reduce to the existing three substrates, tentatively named S_working, Reasoned Hypothesis only, falsification test (EXP-019) proposed but not run.
