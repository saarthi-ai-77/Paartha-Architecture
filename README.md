...
**Current Milestone:** ACA-MVP-001 Benchmark A, complete including its stretch ablation (`docs/11_mvp/ACA-MVP-001.md`, Section 8) — the first experiments in this program to use a real Transformer rather than an MLP. EXP-018 tested whether competence-gated memory (ME-03, validated at toy scale by EXP-001) protects old facts from catastrophic forgetting under staged, non-rehearsed continual training: **the pre-registered success criterion (≥2x baseline retention) failed cleanly** — memory coverage of the oldest facts was exactly 0.0 at evaluation time, in every seed, for both naive and competence-gated policies. EXP-010 then tested the most direct candidate fix (a one-time consolidation-replay burst at each stage boundary) — **also failed** (0.160 ± 0.028 vs. 0.158 ± 0.027 baseline, no improvement). Both mechanisms fully identified, not just observed: `docs/06_experiments/Completed.md`. Benchmark B (SCAN) is next; interleaved rehearsal and explicit weight-protection remain open, untested, larger candidates for ME-03.
