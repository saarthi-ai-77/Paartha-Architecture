...
**Current Milestone:** EXP-009 complete — **falsified ACA v0.4's central assumption that EVALUATE is a single function.** It splits into EVALUATE-LOCAL (per-instance confidence — validated, label-free realizable via entropy/ensemble/self-assessment, all matching the true-label oracle) and EVALUATE-GENERALIZATION (does this generalize or merely fit what's been seen — no label-free realization tested was adequate; entropy and a self-assessment head failed completely at computation-family selection, in every seed, fooled by overfitting exactly as EXP-003 already showed raw training loss is). ACA v0.4 (`docs/04_architecture/ACA_v0.4_Architecture.md`) and ARS-001 (`docs/08_requirements/ARS-001.md`, Section 6) are both revised accordingly, with the original hypothesis preserved rather than edited.
**Current Architecture Status:** Designed (ACA v0.4), not yet implemented as an integrated system. Components validated by experiment: episodic memory allocation (EXP-001), rule/family selection (EXP-002/003), their composability (EXP-004), and EVALUATE-LOCAL's label-free generality (EXP-009). Open, high-priority gap: whether any label-free proxy for EVALUATE-GENERALIZATION exists at all (EXP-013) — without one, computation-selection and deployment-readiness require real held-out labels, not a self-contained confidence signal.
