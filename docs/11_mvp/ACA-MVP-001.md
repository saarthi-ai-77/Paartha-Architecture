...
**Stretch ablation result (same day, EXP-010, `docs/06_experiments/Completed.md`): also failed.** A one-time replay burst (20 extra gradient steps per stage boundary, reinforcing memory's contents directly into the backbone before eviction) produced no measurable improvement — 0.160 ± 0.028 vs. 0.158 ± 0.027 baseline, well within noise. Reasoned mechanism: a brief burst is not defended against the 800 steps of unrelated training that immediately follow it. **Benchmark A's full ablation set (core + stretch) is now complete, and both the original mechanism and its most direct minimal fix are falsified.** Per this document's own §7 publication roadmap, this is the honest, still-publishable negative result: a real-Transformer-scale test of the memory-allocation-policy hypothesis, what specifically broke it, and why the first candidate fix didn't either. Benchmark B (SCAN) is now the appropriate next step — it tests an independent claim (structure-matched composition, EXP-002) unaffected by ME-03's outcome — while interleaved rehearsal and explicit weight-protection remain open, larger, untested candidates for ME-03 specifically, tracked in the roadmap rather than blocking Benchmark B.

## 9. Benchmark B Result (Appended, Not Edited Into Sections 1–7's Pre-Registered Design)

**Result: the §5 success criterion is exceeded overwhelmingly, not marginally.** Structure-matched COMPOSE (fixed, real-data-verified SCAN grammar + a 326-parameter learned primitive lookup) scores **100.000% ± 0.000%** exact-sequence-match on all 7,706 real held-out `addprim_jump` test examples, across 5 seeds. A real encoder-decoder Transformer (681,481 parameters) trained end-to-end on the identical data scores **0.71% ± 0.39%** — closely matching Lake & Baroni (2018)'s own published ~1% figure for vanilla seq2seq models on this exact split, confirming the baseline is a real, literature-consistent comparison, not a weakened strawman. Relative improvement: **~141x**, against a pre-registered threshold of 2x. Parameter reduction: **~2090x fewer parameters**, with better accuracy, not a tradeoff. Full write-up, grammar verification methodology, and data: `docs/06_experiments/Completed.md` (EXP-020).

**Why this is not a surprising or suspicious result:** the compositional grammar was verified against **all 22,376** real train+test examples (not a sample) before any model was written, and the only learned quantity is a 4-class lookup (walk/look/run/jump → action token) extracted from genuine training exposures — "jump" occurs 1467 times in isolation in the real training data, never composed, exactly matching the split's defining premise. Given a fully-verified fixed grammar, correct generalization to unseen compositions is the deterministic, expected consequence, not luck — precisely mirroring EXP-002's own stage-5 result (100% on true rotations) at real benchmark scale instead of a 40-example synthetic task.

**The caveat is unchanged from EXP-002, not new:** this says nothing about how a system would discover the correct grammar without a human specifying it — `docs/11_mvp/ACA-MVP-001.md` §2 excluded automatic/learned routing (RC-02) from this benchmark for exactly that reason. Benchmark B confirms structure-matched COMPOSE (RC-01) generalizes correctly *given* correct structure, at real scale; it does not touch the family-*discovery* problem EXP-002 named as the central open question, still unresolved (EXP-005, not yet run).

**Status of ACA-MVP-001 as a whole:** Benchmark A (ME-03, memory) is a clean, mechanistically-understood negative result. Benchmark B (RC-01, compose) is a clean, decisive positive result. Both are honestly reported as what they are — this document's own discipline does not average them into a single verdict on "ACA": the two components have genuinely different, independently-earned outcomes so far.

## 10. Benchmark C Result (Appended, Not Edited Into Section 3's Pre-Registered Design)

**Result: composability confirmed at real scale, extending EXP-004's toy-scale finding.** One joint model (Benchmark A's recall pathway + Benchmark B's structure-matched compose pathway, disjoint parameters, one optimizer, mixed batches every step) shows **no measurable interference**: recall Stage-1 accuracy 0.152 ± 0.025 (vs. 0.158 ± 0.027 isolated, EXP-018) and compose exact-match 1.0000 ± 0.0000 (vs. 1.0000 ± 0.0000 isolated, EXP-020) — both within noise or an exact match. Full write-up: `docs/06_experiments/Completed.md` (EXP-021).

**What this changes and doesn't:** ME-03's independent limitation (EXP-018/010) is unaffected — memory coverage was 0.000 in every seed here too, exactly as before. This experiment answers a narrower, different question (does combining pathways hurt either one) and answers it cleanly: no. RC-01's real-scale success (EXP-020) also survives joint training exactly, not just in isolation.

**ACA-MVP-001's originally-scoped Benchmark A/B/C implementation order (§ "Implementation Order" above) is now complete.** Three independently-earned outcomes: memory (negative, mechanistically understood, two candidate fixes also ruled out), compose (decisively positive, at real published-benchmark scale), composability (confirmed, real scale, disjoint parameters). None of the three outcomes is averaged into the others.
...
**Current Status:** Complete. Benchmark A, including its stretch ablation — pre-registered success criterion failed (EXP-018), most direct candidate fix also failed (EXP-010). Benchmark B — pre-registered success criterion exceeded decisively (EXP-020, ~141x vs. a 2x threshold). Benchmark C — composability confirmed at real scale, no interference (EXP-021).
**Historical Context:** Produced 2026-07-18 under a Chief Experimental Scientist role, following DAS-001, in response to a directive that further conceptual architecture work was producing diminishing returns without implementation evidence. Benchmark A and its consolidation stretch ablation executed and logged 2026-07-22. Benchmark B executed and logged 2026-07-23, in parallel with a separate CTO-directed research track (CTX-001, `docs/12_cognition/CTX-001.md`). Benchmark C executed and logged 2026-07-23, after EXP-019 resolved per explicit CTO sequencing.
**Known Facts:** Every included component traces to EXP-001, EXP-002, or EXP-009; every excluded component traces to an explicitly still-unvalidated hypothesis (EXP-005/012/014/015). Results: EXP-018, EXP-010 (Benchmark A, negative), EXP-020 (Benchmark B, decisively positive), EXP-021 (Benchmark C, composability confirmed) — all in `docs/06_experiments/Completed.md`.
**Hypotheses:** Benchmark A's hypothesis (§5) failed, its stretch ablation's hypothesis also failed (Section 8). Benchmark B's hypothesis (§5) succeeded overwhelmingly (Section 9) — with the same "structure must still be hand-specified" caveat EXP-002 already raised, not newly discovered. Benchmark C's hypothesis (no interference) confirmed (Section 10).
**Unknowns:** For A: whether ME-03 survives staged continual training under any fix remains open. For B: whether structure can ever be discovered rather than hand-specified (EXP-005, unchanged, still not run) — the central open problem, recurring at scale rather than resolved by it. For C: shared-substrate interference and learned routing, both untested (Section 10 follow-up).
**References:** `docs/04_architecture/ACA_v1.0_Architecture.md`, `docs/09_validation/IVS-001.md`, `docs/10_deployment/DAS-001.md`, `docs/06_experiments/Completed.md` (EXP-001, EXP-002, EXP-009, EXP-018, EXP-010, EXP-020, EXP-021), `docs/12_cognition/CTX-001.md`, `docs/13_state_model/SOS-001.md`
