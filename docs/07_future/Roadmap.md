...
**ARS-001 (`docs/08_requirements/ARS-001.md`) now gates architecture design, not experimentation.** Per the directive that produced it, no concrete architecture should be designed until this specification is considered stable — but EXP-005 through EXP-008 remain component-level validation work, not architecture design, so they are not blocked by it. ARS-001 does, however, reframe which of them is highest-value: it identified **SR-01 (a calibrated, causally-active self-evaluation signal)** as the single most load-bearing requirement in the entire specification — a dependency of memory consolidation, family selection, planning, and deployment readiness alike — while noting that ACA has only validated a narrow, fixed-threshold instance of it (EXP-001's surprise gate), not a general-purpose version. It also flagged the consolidation-transfer half of the memory pipeline (moving episodic content *into* semantic weights, not just evicting mastered entries) as a real, currently untested gap (ME-03 in ARS-001).
