...
`docs/14_integration/SIP-001.md` Section 19 built and ran the first end-to-end integrated runtime combining ME-03 (memory), EVALUATE-LOCAL (SR-01's entropy signal), and RC-01 (compose) behind one request pipeline. Per that document's own revision policy, an Architectural Limitation is escalated here as a new finding, not folded into Sections 6/7/11 as if always known:

**No prior experiment (EXP-001, EXP-018, EXP-010) ever tested single-exposure factual teaching.** Every one trained each fact with hundreds of repeated gradient steps within a stage before evaluating. SIP-001's integration test taught 40 facts with exactly one gradient step each (a real, ordinary usage pattern — a user stating a fact once in conversation) and found: entropy never left a narrow 3.7–4.2 nat band (never approaching the fixed 1.5-nat confidence threshold used for Knowledge Boundary Detection), and the gated-write memory policy, faced with entropy readings clustered this tightly, wrote only 4 of 40 facts (116 of 136 attempted writes skipped as "not surprising enough" — not evicted, never written in the first place). A same-code control with 30 repeated exposures per fact recovered 100% recall accuracy entirely through the backbone's own confidence, confirming this is not an implementation defect but a genuine boundary condition: **the current entropy-threshold tuning implicitly assumes a repeated-exposure training regime and degrades under single-exposure teaching.**

This directly confirms, rather than newly discovers, a caveat this document's own Section 2.2a already carried: SR-01's fixed threshold is "a fixed hyperparameter... not a general-purpose, calibrated signal usable across arbitrary inputs" (per `docs/08_requirements/ARS-001.md`'s original SR-01 entry). SIP-001's integration test is what made that caveat concrete and measured rather than a stated possibility.

**Not resolved here, per this document's own Section 15 policy and SIP-001's Core Principle (no architecture change without demonstrated necessity, and even then not invented ad hoc):** whether an exposure-count-aware gating rule fixes this is a genuinely new, unattempted experiment — named as follow-up, not solved by this appendage.

Full detail: `docs/14_integration/SIP-001.md` §19, `runtime/sip001/`.
...
**Current Status:** Active — v1.0, synthesized, now integration-tested end-to-end (`docs/14_integration/SIP-001.md`). Audited once (Section 17), corrected/extended five times against real-scale/integration evidence (Section 18: EXP-018/010/020/019; Section 19: SIP-001's single-exposure-teaching finding).
**Historical Context:** Produced 2026-07-18 under a changed role (architecture synthesis rather than research), immediately following ARS-001 Section 7's required theoretical clarification on structural evaluation. Audited the same day per `docs/09_validation/IVS-001.md`'s prerequisite requirement. Section 18 added 2026-07-22 following ACA-MVP-001 Benchmark A's completion, extended same day with EXP-010, extended again 2026-07-23 with EXP-020 (Benchmark B) and EXP-019 (S_working resolution). Section 19 added 2026-07-23 following SIP-001's first integration test, under a further changed role (Chief Systems Engineer).
**Known Facts:** See Section 2's per-function evidence tags and Section 11's falsification table (now including EXP-018's ME-03 scope-narrowing and EXP-010's falsified minimal fix). RC-01/COMPOSE's real-scale confirmation (EXP-020) does not change its evidence tag (already Validated) but substantially extends its evidentiary base. The state model (Section 1) remains three substrates, not four — EXP-019 resolved the candidate fourth (S_working) via a disciplined S_episodic partition instead, catalogued in `docs/13_state_model/SOS-001.md`. SR-01's fixed-threshold caveat (already stated in ARS-001) is now concretely confirmed by integration testing, not merely a stated possibility (Section 19).
**Hypotheses:** Every component tagged Reasoned hypothesis or Speculative throughout, most consequentially S_invariants (1.3) and EVALUATE-STRUCTURAL (2.2c), both contingent on EXP-015. ME-03's general-case status is genuinely open — its most direct candidate fix (one-time consolidation replay) is now also falsified; interleaved rehearsal and explicit weight-protection are the next, untested, structurally larger candidates. RC-01's confirmation at scale does not touch RC-02's still-fully-open automatic family-discovery problem (EXP-005). Whether an exposure-count-aware gating rule resolves Section 19's finding is untested.
**Unknowns:** Section 12's six experiments (now superseded in priority order by IVS-001 §6's blast-radius ranking, itself due for revision per Section 18); Section 13's five named risks, especially whole-system integration, now partially attempted (SIP-001) rather than never attempted. Whether EXP-019's write-starvation mechanism also affects the routing or self-model schemas, or the episode schema under real capacity pressure (SIP-001's integration test never actually filled memory to capacity) — untested.
**References:** `docs/08_requirements/ARS-001.md` (Sections 2, 5, 6, 7), `docs/06_experiments/Completed.md` (EXP-001–004, EXP-009, EXP-018, EXP-010, EXP-020, EXP-019), `docs/04_architecture/ACA_v0.4_Architecture.md` (superseded, preserved), `docs/09_validation/IVS-001.md`, `docs/11_mvp/ACA-MVP-001.md`, `docs/12_cognition/CTX-001.md`, `docs/13_state_model/SOS-001.md`, `docs/14_integration/SIP-001.md`, `docs/03_foundations/{ACCEPTED,REJECTED,OPEN}.md`
