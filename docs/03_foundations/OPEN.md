...
* **ACA v1.0 (`docs/04_architecture/ACA_v1.0_Architecture.md`):** the full architecture specification, derived from ARS-001, fully traced (function → requirement → evidence). Synthesized, not yet implemented as an integrated system. EVALUATE is now formalized as **three** realizations: LOCAL (validated), GENERALIZATION (validated only with real labels; falsified label-free by EXP-009), and **STRUCTURAL** (new, `docs/08_requirements/ARS-001.md` §7 — verification against known domain invariants; a reasoned hypothesis, not yet tested). **EXP-015** (does structural evaluation correctly reject a non-generalizing family, label-free?) is now the highest-priority unresolved question — it tests whether v1.0's new S_invariants substrate has any justification at all. EXP-013 (any other label-free proxy for GENERALIZATION), EXP-014 (where RC-04 falls), EXP-010 (consolidation via replay), EXP-011 (routing-as-episodic-content), and EXP-012 (novel-input fallback) are the other named gaps — full detail in `ACA_v1.0_Architecture.md` Section 12.
