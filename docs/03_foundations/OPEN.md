...
* **ACA v0.4 (`docs/04_architecture/ACA_v0.4_Architecture.md`):** the first concrete architecture, derived from ARS-001, fully traced (function → requirement → evidence). Designed, not yet implemented as an integrated system. Its most consequential open question — whether EVALUATE (the function both SELECT and UPDATE depend on) generalizes beyond EXP-001's narrow, fixed-threshold instance into a general-purpose signal usable without ground truth — is the most important unresolved question in the entire research program, tracked as EXP-009. EXP-010 (explicit consolidation via replay), EXP-011 (routing-as-episodic-content), and EXP-012 (novel-input fallback) are the other three named gaps.
