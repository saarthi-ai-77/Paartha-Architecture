...
* **ARS-001 (`docs/08_requirements/ARS-001.md`):** Architecture Requirement Specification, now complete through two reduction passes — ~30 candidate mechanisms → 15 requirements in 5 domains (Section 1) → 4 fundamental functions, EVALUATE/SELECT/UPDATE/COMPOSE (Section 5). Not yet formally reviewed or frozen, but architecture design may now begin per Section 5.9. Its most consequential open question — whether EVALUATE (the function underlying SR-01, and the one function both SELECT and UPDATE depend on) generalizes beyond EXP-001's narrow, fixed-threshold instance into a general-purpose signal — is currently the most important unresolved question in the entire research program, ahead of EXP-005 through EXP-008.
