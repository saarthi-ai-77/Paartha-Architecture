...
* **CTX-001 (`docs/12_cognition/CTX-001.md`):** investigates whether memory (EXP-018/010), conversation context, and "known unknown" tracking share a root cause, per a CTO directive questioning their independence. Finds the *strong* version of "cognitive state, not information, is ACA's primary object" unsupported (relabeling doesn't fix EXP-018's retention-under-sequential-pressure failure) but a *narrower* version defensible: conversation/reasoning context is the one candidate that does not cleanly reduce to the three existing substrates (directly extending an open question ARS-001 §2 already posed and IVS-001's audit never actually resolved), tentatively named **S_working**, Reasoned Hypothesis only, with a falsification test (EXP-019) proposed but not run. The "unknown" pipeline mostly reduces to existing functions plus a new scheduled orchestration process ("Gap Curriculum Generation," offered to DAS-001, not self-adopted). No new function or substrate has been added to ACA v1.0 as a result of this document.
