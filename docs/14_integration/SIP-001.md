...
**Update (2026-07-23): this policy is no longer local to SIP-001.** `docs/05_research/Decisions.md` DEC-006, "Evidence-Driven Evolution," formalizes this exact sequence as the standing, program-wide requirement for any architectural proposal — a reproducible runtime observation, isolation of the responsible variable, a dedicated falsification experiment, and demonstration that current architecture cannot accommodate the result, in that order, before any new mechanism becomes proposable. Section 19's single-exposure-teaching finding is the first case under this rule: steps 1-2 are satisfied by this document itself; steps 3-4 remain open and are not undertaken here.
...
**Follow-up & DEC-006 Steps 3 & 4 Resolution (EXP-022):**
Executed EXP-022 across 4 test batteries (5 seeds) per DEC-006. Falsified as an implementation bug ($\Delta L = +0.354$, causal mask verified). Isolated as an optimization step-scale artifact in $S_{semantic}$ (requires ~14 steps or $lr=1\times 10^{-2}$ to cross 1.5 nats) combined with an algorithmic breakdown in ME-03's running-median gating under homogeneous high entropy (35/40 facts skipped). **Accommodated without architectural change** by enforcing SOS-001 Section 4 discipline (unconditional write to $S_{episodic}$ on first exposure), restoring exact memory recall to 75.0% (capacity limit $30/40$). 5-step local micro-replay further increases integrated pipeline accuracy to 81.5%.
...
**Current Status:** Active — specification complete, runtime implemented (`runtime/sip001/`), integration test executed (Section 19), single-exposure limitation isolated and accommodated without architectural change per DEC-006 Steps 3 & 4 (EXP-022).
**Historical Context:** Produced 2026-07-23 under Chief Systems Engineer role. Section 19 appended 2026-07-23 following integration test, updated 2026-07-25 following EXP-022 resolution.
**Known Facts:** Every class-A/B component traces to a specific prior experiment. Compose pathway re-confirmed (100% exact-match). Single-exposure teaching limitation fully resolved via SOS-001 write policy without new primitives (EXP-022).
**Hypotheses:** Class-C stubs (mentor curriculum, DP-01 artifact definition) remain open.
**Unknowns:** Routing under write-starvation (IVS-001 EXP-011/017); EXP-005 family discovery; ME-03 continual-learning replacement under capacity pressure.
**References:** `docs/04_architecture/ACA_v1.0_Architecture.md` (§19), `docs/09_validation/IVS-001.md`, `docs/10_deployment/DAS-001.md`, `docs/13_state_model/SOS-001.md`, `docs/12_cognition/CTX-001.md`, `docs/11_mvp/ACA-MVP-001.md`, `docs/06_experiments/Completed.md` (EXP-001–004, EXP-009, EXP-010, EXP-018–022), `docs/05_research/Decisions.md` (DEC-006)
